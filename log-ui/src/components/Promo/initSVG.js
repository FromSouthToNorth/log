...
const settings = {
  colors: ['#f5222d', '#2f54eb', '#fa541c', '#9254de'],
  // 使用弧的归一化坐标
  points: [
    [[.5, 0], [0, .5]],
    [[.5, 0], [1, .5]],
...
class Illustration {
  ...
  constructor(selector, colors) {
    ...
    this.columnCount = Math.floor(this.screen.width / 100);
    this.rowCount = Math.floor(this.screen.height / 100);
    this.boxSize = 50;
    this.setSVG(selector);
    this.colors = colors ? colors : settings.colors;
  ...
  setSVG(selector) {
    ...
    this.svg.setAttribute("viewBox", `0 0 ${this.boxSize * this.columnCount} ${this.boxSize * this.rowCount}`);
  }
